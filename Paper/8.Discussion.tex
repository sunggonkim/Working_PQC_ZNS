...
Zone reset alone is not physical NAND erasure; strong erasure requires sanitize or hardware crypto-erase, and those commands may be device- or namespace-scoped with deployment-specific SLO cost.  The physical evidence uses one WD ZN540-class device, zonefs replay, liboqs/OpenSSL traces, a TLS socket trace, a Sysbench/MySQL execution gate, and an xNVMe probe.  Exact DOGI/MiDAS/SepBIT runs stay separate because their units differ from packed actual-\zns replay.  We report host-visible resets/utilization, but not internal wear telemetry, full application p99 latency, real DB block traces, FDP hardware behavior, or full SPDK poll-mode replay.
