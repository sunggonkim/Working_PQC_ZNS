...
| Final `HowToWritePaper.md` audit. | `LINE_BY_LINE_FAST_AUDIT.md` now checks the guide's 10 pre-submission audit questions and the design-choice rule directly. Figure 4 was visually checked after label compaction, and Figure 6 visualizes WAF vs. utilization. | Satisfied. |
...
| --- | --- | --- |
| Figure polish | FAST reviewers read plots before prose. Figure 4 label readability is fixed, and the WAF-vs-utilization figure now visualizes the space-amplification tradeoff. | Checked for current build. |
| Table-heavy Evaluation | The paper still carries exact measured matrices in tables, but the main reviewer attacks now have figure paths: pressure, hardness, oracle bound, architecture, and WAF-vs-utilization. | Intentional auditability tradeoff. |
...
| Final figure/caption inspection | Current visual pass checked the Figure 4 page after label compaction. Repeat only if figures move or new plots are added. | Checked for current build. |
...
| "Did QUASAR buy WAF by wasting zones?" | Component Ablation reports WAF/space sensitivity: in Exchange p2000, QUASAR lowers WAF and stale secrets while keeping closed-zone fill at 94.1--96.1%, with a visible lifetime-utilization cost. |
...
| First evidence figure. | Actual-ZNS YCSB pressure figure appears immediately after the opening gap. | Good. |
...
| Ablation. | Explicit component ablation table. | Good. |
| Sensitivity/robustness. | Bad hints, tenants, residual migration, open-zone budget. | Good. |
| Cost and limitation. | C-level routing cost, zonefs/xNVMe caveat, sanitize boundary, device/wear caveat. | Good. |
