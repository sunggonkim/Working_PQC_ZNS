...
\caption{Actual-\zns YCSB pressure curve.  The low-pressure row is intentionally a negative WAF control: WAF is near 1.0 for history-based and hybrid policies, but expired secrets remain in non-resettable baseline zones.  Higher PQC ratios create GC/WAF pressure while preserving the same semantic gap.}
