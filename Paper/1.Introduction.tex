...
\caption{PQC lifecycle pressure on actual \zns.  The left panel shows the PQC artifacts that create the storage problem: epoch secrets, rotation metadata, and append-only signatures.  The right panel shows the consequence after epoch close: FIFO, \sepbit-style, \midas-style, and \dogi-style placement strand expired PQC secret blocks, while \system-\dogi keeps those death cohorts reset-eligible.}
