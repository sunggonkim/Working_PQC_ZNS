...
| "Physical erase overclaim" | Zone reset is not NAND erase, and shared-namespace sanitize can destroy unrelated data. | Paper says this directly and separates sanitize/crypto-erase command-path evidence from per-zone erase. | Dedicated namespace/media pool, per-cohort key isolation, or future per-zone erase semantics before making a stronger erase claim. |
