...
| Configuration sensitivity | Open-zone budget, binning, missing/wrong hints, residual cleanup cost, and FDP handle pressure. | `fig:open-zone-robustness`, `fig:fdp-handle-pressure`. |
...
| Secure erase overclaim | Zone reset is not universal physical erasure, and shared-namespace sanitize is not per-zone cleanup. | Paper separates reset eligibility from destructive device/namespace-scoped sanitize evidence and adds per-cohort key destruction as cohort-scoped crypto-erase evidence. | Hardware per-zone erase or dedicated namespace/media-pool evidence before making a stronger physical-erasure claim. |
...
| Interface boundary | Explain whether QUASAR is ZNS-only. | Background table and Discussion distinguish native ZNS measurement from FDP deployment mapping; Evaluation adds handle-pressure evidence. | Pass | No physical FDP hardware result is claimed. |
...
| "Physical erase overclaim" | Zone reset is not NAND erase, and shared-namespace sanitize can destroy unrelated data. | Paper says this directly, separates sanitize/crypto-erase command-path evidence from per-zone erase, and validates per-cohort DEK destruction as a cohort-scoped crypto-erase path. | Hardware per-zone erase or dedicated namespace/media-pool evidence before making a stronger physical-erasure claim. |
| "FDP would solve this more cleanly" | Native ZNS may look overfit. | Paper positions QUASAR as the lifecycle signal and FDP/ZNS as two carriers; the handle-pressure model shows scarce handles can collide death cohorts. | Physical FDP replay when available. |
