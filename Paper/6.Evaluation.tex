...
\begin{table*}[t]
\centering
\caption{Representative actual-\zns PQC lifecycle pressure on YCSB carriers.  The carrier preserves update locality; the PQC side writes create protocol-defined death cohorts that history-based placement cannot observe.}
\label{tab:ycsb}
\footnotesize
...
\begin{table*}[t]
\centering
\caption{PQC lifecycle pressure across DB and dynamic-service carriers.  Each row preserves a DOGI-style storage-locality pattern, then adds PQC artifacts whose death time is protocol-defined; the hybrid preserves zero stale secrets while cutting GC.}
\label{tab:pressure}
\footnotesize
\begin{tabularx}{\textwidth}{lRRRRRRR}
\toprule
...
\begin{figure*}[t]
\centering
\subfloat[GC cost from missing lifecycle signal.]{
\includegraphics[width=0.48\textwidth]{../artifacts/figures/fast-style/fig3-component-ablation-gc.pdf}
}
\hfill
\subfloat[Expired PQC secrets after epoch close.]{
\includegraphics[width=0.48\textwidth]{../artifacts/figures/fast-style/fig3-component-ablation-secrets.pdf}
}
\caption{PQC lifecycle-signal ablation.  A history-only \dogi-style policy sees storage history but misses cryptographic death cohorts, so GC and expired-secret exposure remain.  Lifecycle hints remove exposure; adding \dogi-style payload fallback recovers payload placement and removes the remaining GC in the default hybrid.}
...
\centering
\subfloat[Space cost.]{
\includegraphics[width=0.31\textwidth]{../artifacts/figures/fast-style/fig4-resource-space.pdf}
}
\hfill
\subfloat[Policy-decision cost.]{
\includegraphics[width=0.31\textwidth]{../artifacts/figures/fast-style/fig4-resource-decision.pdf}
}
\hfill
\subfloat[xNVMe append path.]{
\includegraphics[width=0.31\textwidth]{../artifacts/figures/fast-style/fig4-resource-xnvme.pdf}
}
\caption{PQC placement resource and overhead controls.  \system-\dogi bounds the closed-zone fill cost while removing stale PQC secrets, the lifecycle-hint routing path is orders of magnitude cheaper than the \dogi-style policy-decision microbenchmark, and xNVMe confirms a lower-overhead append command path than the zonefs helper used for full-suite replay accounting.}
...
\begin{figure*}[t]
\centering
\subfloat[Secrets waiting under bounded active zones.]{
\includegraphics[width=0.31\textwidth]{../artifacts/figures/fast-style/fig6-robustness-waiting.pdf}
}
\hfill
\subfloat[Physical WAF of strict mode.]{
\includegraphics[width=0.31\textwidth]{../artifacts/figures/fast-style/fig6-robustness-waf.pdf}
}
\hfill
\subfloat[Residual migration copy cost.]{
\includegraphics[width=0.31\textwidth]{../artifacts/figures/fast-style/fig6-robustness-copy.pdf}
}
\caption{PQC robustness frontier.  Exact and binned placement can leave delayed-expiry PQC secrets waiting under tight active-zone budgets.  Residual migration removes the exposure but increases physical WAF and copied blocks; strict zero-wait mode is therefore a policy point, not the default.}
