...
\begin{figure*}[t]
\centering
\includegraphics[width=0.82\textwidth]{../artifacts/figures/actual-zns/workload-hardness.png}
\caption{Workload hardness guardrail.  The evaluation separates DOGI-favorable controls, negative WAF controls, pressure workloads, claim-gate rows, and QUASAR-hostile robustness cases.  This prevents clean PQC-only traces or easy YCSB rows from becoming the headline WAF evidence.}
\label{fig:workload-hardness}
\end{figure*}
...
\begin{figure*}[t]
\centering
\includegraphics[width=0.78\textwidth]{../artifacts/figures/actual-zns/ycsb-pressure-waf-stale.png}
\caption{YCSB pressure curve on actual \zns.  The p2000 row is a negative control; p4000 through p10000 show where pressure creates GC/WAF separation.  Across all rows, the stale-secret gap remains visible.}
...
Table~\ref{tab:ablation} turns the upper-bound requirements into mechanism evidence.  History-only placement is the right first row because it gives \dogi-style placement all storage-visible signals but no protocol lifetime.  Adding lifecycle hints removes stale secrets, but pure semantic grouping can still leave GC cost on DB-like payload.  Adding the \dogi payload fallback removes that remaining cost.  Admission and residual migration are not free performance wins; they are boundary mechanisms for open-zone pressure and stragglers.  Figure~\ref{fig:space-sensitivity} visualizes the space tradeoff: tight-budget hybrids lower WAF and stale secrets without relying on empty zones.

\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{../artifacts/figures/actual-zns/space-sensitivity.png}
\caption{WAF vs. utilization sensitivity on Exchange p2000.  \system-\dogi candidates keep stale secrets at zero with closed-zone fill 0.936--0.961, while the \dogi-style point keeps higher fill but leaves 3,748 stale secret blocks.}
\label{fig:space-sensitivity}
\end{figure}
