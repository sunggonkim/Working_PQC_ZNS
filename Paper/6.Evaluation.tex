...
\subsection{PQC Lifecycle Pressure on YCSB Carriers}
...
\subsection{PQC Lifecycle Pressure Across Service Carriers}
...
\subsection{Open-Zone and Hint Robustness}
...
\begin{figure*}[t]
\centering
\subfloat[Live zones versus the device open-zone limit.]{
\includegraphics[width=0.31\textwidth]{../artifacts/figures/fast-style/fig6-open-zone-zones.pdf}
}
\hfill
\subfloat[Final waiting secrets under bad hints and stragglers.]{
\includegraphics[width=0.31\textwidth]{../artifacts/figures/fast-style/fig6-open-zone-waiting.pdf}
}
\hfill
\subfloat[Physical WAF cost of strict residual cleanup.]{
\includegraphics[width=0.31\textwidth]{../artifacts/figures/fast-style/fig6-open-zone-waf.pdf}
}
\caption{Open-zone and hint robustness, matching DOGI's configuration-sensitivity role.  Clean, missing-hint, and wrong-epoch rows complete within the device budget.  Delayed-expiry stragglers expose the hard boundary: exact epoch placement can exceed the open-zone limit, binning fits the limit but leaves waiting secrets, and residual migration removes the waiting secrets at explicit physical-WAF cost.}
\label{fig:open-zone-robustness}
\end{figure*}

\subsection{Prototype Overhead}
...
\begin{figure*}[t]
\centering
\subfloat[Actual-\zns replay throughput with zonefs helper accounting.]{
\includegraphics[width=0.48\textwidth]{../artifacts/figures/fast-style/fig7-prototype-throughput.pdf}
}
\hfill
\subfloat[C-level placement-decision cost across PQC traces.]{
\includegraphics[width=0.48\textwidth]{../artifacts/figures/fast-style/fig7-decision-cost.pdf}
}
\caption{Prototype overhead in the role of DOGI's prototype figure.  Full-suite actual-\zns replay includes zonefs helper overhead and semantic reset commands, so it is an accounting bound rather than an optimized SPDK datapoint.  The isolated C-level decision benchmark shows that lifecycle-hint routing is much cheaper than \dogi-style feature extraction and MLP scoring, while the hybrid keeps the DOGI fallback only where payload history is useful.}
